...
void RST2Matrix(RST *a,MATRIX &m, bool kill)
{
	...
	{
		MATRIX b;
		M_Scale(KillFloat(a->Scale.x,2),KillFloat(a->Scale.y,2),KillFloat(a->Scale.z,2),m);
		QUATERNION c=a->Quaternion;
		c.s=KillFloat(c.s,2);
		c.v.x=KillFloat(c.v.x,2);
		c.v.y=KillFloat(c.v.y,2);
		c.v.z=KillFloat(c.v.z,2);
		M_Quaternion(a->Quaternion,b);
		M_Mult(b,m,m);
		M_XPose(KillFloat(a->Pos.x,2),KillFloat(a->Pos.y,2),KillFloat(a->Pos.z,2),b);
		M_Mult(b,m,m);
	}
	else*/
	{
