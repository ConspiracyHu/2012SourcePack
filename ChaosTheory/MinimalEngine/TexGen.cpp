...
/*void TEXTURE::Jpeg(RGBA* Layer, COMMAND* Parameters) 
{
	...
	if (r->JPEGData)
	{
		...
		if (picType==PICTYPE_METAFILE || picType==PICTYPE_ENHMETAFILE) 
		{
			...
			XSize=XRes;
			YSize=(int)(hmHeight/(float)hmWidth*(float)XSize);
		}
		else AAlevel=1;
...
void TEXTURE::Jpeg(RGBA* Layer, COMMAND* Parameters) 
{
	...
	//if (r->JPEGData)
	{
		...
		gpPicture->Render(mdc, 0, 0, XSize, YSize, 0, hmHeight, hmWidth, -hmHeight, &r);

		unsigned char *buffer = new unsigned char[XSize*YSize*4];
		memset(buffer,0,XSize*YSize*4);

		GetDIBits(mdc,bm,0,YSize,buffer,&bmi,DIB_RGB_COLORS);

		int xs4=TextureX;
		int cols[4];
		int l=(YSize-1)*XSize*4;
		int cx4aad2=XSize*4;
		int cxat4=XSize*4;
		int p,jj,z,a=0;
